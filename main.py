...
while not game_over:
    ...
    if turn == AI and not game_over:
        ...
        if x == 'n':
            if y == "beginner":
                col, minimax_score = minimax(board, 2, False)
            if y == "intermediate":
                col, minimax_score = minimax(board, 3, False)
            if y == "advanced":
                col, minimax_score = minimax(board, 4, False)
        elif x == 'y':
            if y == "beginner":
                col, minimax_score = minimax_Alpha_Beta(
                    board, 1, -math.inf, math.inf, False)
            if y == "intermediate":
                col, minimax_score = minimax_Alpha_Beta(
                    board, 4, -math.inf, math.inf, False)
            if y == "advanced":
                col, minimax_score = minimax_Alpha_Beta(
                    board, 5, -math.inf, math.inf, False)
        else:
            print("Enter the type of algorithm")
